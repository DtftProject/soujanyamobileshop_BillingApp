...
const stockManagement = (addEditType, addEditItem, fieldsVar) => {
	...
	const recalculateFinalTotal = () => {
		...
		total = total - formData.value.exchange;
		const tax = total * (taxRate / 100);

		total = total + parseFloat(formData.value.shipping);

		formData.value.subtotal = formatAmount(total + tax);
		formData.value.subtotalwithoutround = formatAmount(total + tax);
		roundprice();
		formData.value.tax_amount = formatAmount(tax);
		formData.value.discount = discountAmount;
	};
	const roundprice = () =>{  
        let roundfigure = formData.value.roundfigure!= undefined
                ? parseInt(formData.value.roundfigure)
                : 0;
        let price = formData.value.subtotalwithoutround;
        switch (roundfigure) {
          case 2:

            formData.value.subtotal = Math.floor(price);
            formData.value.adjustment  = Math.floor(price)-price;
            break;
          case 1:
            formData.value.subtotal = Math.ceil(price);
            formData.value.adjustment  = Math.ceil(price)-price;
            break;
          case 'none':
          default:
            // Leave as is
	...
	const editItem = (product) => {
		 addEditFormData.value = {
            id: product.xid,
            item_code:product.item_code?product.item_code:product.product_details.item_code,
            discount_rate: product.discount_rate,
            unit_price: product.unit_price,
            tax_id: product.x_tax_id,
            category_id: product.category_id?product.category_id:product.product_details.category_id,
	...
	const onAddEditSubmit = () => {
		...
		console.log(selecteTax,taxType);
		...
		const newData = {
			...
			tax_type: taxType,
			item_code:addEditFormData.value.item_code,
            category_id:parseFloat(addEditFormData.value.category_id),
		};
