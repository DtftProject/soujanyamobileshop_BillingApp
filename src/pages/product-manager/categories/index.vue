<template>
    <f7-page
        ...
        @infinite="fetch"
    >
        <f7-navbar :title="$t('menu.categories')" :back-link="$t('common.back')">
            <f7-searchbar
                :custom-search="true"
            ...
            <f7-card-content :padding="false">
                ...
                >
                    <f7-list-item
                        :title="`${item.name}`"
                        swipeout
                        @click="viewItem({ item, categories })"
                    ...
                    >
                        ...
                        <f7-swipeout-actions right>
                            <f7-swipeout-button @click="viewItem({ item, categories })">
                                {{ $t("common.view") }}
                            </f7-swipeout-button>
                            <f7-swipeout-button
                                v-if="
                                ...
                                "
                                @click="
                                    editItem({ item, categories, addEditType: 'edit' })
                                "
                            >
                                {{ $t("common.edit") }}
                            </f7-swipeout-button>
...
export default {
    ...
    setup(props) {
        ...
        onMounted(() => {
            const categoriesPromise = axiosAdmin().get(
                "categories?fields=id,xid,name&order=parent_id asc&limit=10000"
            );

            Promise.all([categoriesPromise]).then(([categoryResponse]) => {
