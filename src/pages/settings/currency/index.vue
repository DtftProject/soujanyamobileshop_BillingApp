<template>
    <f7-page
        ...
        @infinite="fetch"
    >
        <f7-navbar :title="$t('menu.currencies')" :back-link="$t('common.back')">
            <f7-searchbar
                :custom-search="true"
            ...
            <f7-card-content :padding="false">
                ...
                >
                    ...
                    >
                        <template #text>
                            {{ $t("currency.position") }} :
                            {{ $t(`currency.${item.position}`) }}</template
                        >
                        <template #after>
                            <f7-link
                                v-if="
                            ...
                            <f7-swipeout-button
                                v-if="
                                    (permsArray.includes('currencies_delete') ||
                                        permsArray.includes('admin')) &&
                                    appSetting.x_currency_id != item.xid
                                "
                                delete
...
export default {
    ...
    setup(props) {
        ...
        const { formatAmountCurrency, permsArray, appSetting } = common();
